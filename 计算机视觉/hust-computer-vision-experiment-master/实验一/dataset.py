...
def generate_dataset(features: np.ndarray, labels: np.ndarray):
    ...
    :param features: 特征
    :params labels: 标签
    :returns: 训练集 TensorDataset 和 验证集 TensorDataset 的列表
    """
    labels -= min(labels)
    X_train, X_valid, y_train, y_valid = train_test_split(
        features, labels, test_size=params["valid_ratio"], random_state=params["random_seed"], shuffle=True)
    return TensorDataset(torch.tensor(X_train), torch.tensor(y_train)), TensorDataset(torch.tensor(X_valid), torch.tensor(y_valid))
...
if __name__ == "__main__":
    ...
    plt.title("数据分布图")
    sns.scatterplot(x="data1", y="data2", data=raw_data, hue="label")
    ...
    for label in means.index:
        ...
        ax.set_xlabel(r'$x$')
        ax.set_ylabel(r'$y$')
        x = np.linspace(means.loc[label]["data1"] - 6 * stds.loc[label]["data1"],
                        means.loc[label]["data1"] + 6 * stds.loc[label]["data1"], 10000)
        y = np.linspace(means.loc[label]["data2"] - 6 * stds.loc[label]["data2"],
                        means.loc[label]["data2"] + 6 * stds.loc[label]["data2"], 10000)
        X, Y = np.meshgrid(x, y)
        pos = np.dstack((X, Y))
