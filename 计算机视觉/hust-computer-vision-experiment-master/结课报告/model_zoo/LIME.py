"""
@Description :   LIME 算法结构，改写自 https://github.com/marcotcr/lime
@Author      :   tqychy 
@Time        :   2024/01/13 18:52:12
"""
...
class LimeBase(object):
    ...
    def __init__(self,
        ...
        self.random_state = check_random_state(params["random_seed"])
    ...
    def data_labels(self,
        ...
        for row in rows:
            ...
            if len(imgs) == batch_size:
                inputs = torch.tensor(np.array(imgs)).permute(0, 3, 1, 2)
                with torch.no_grad():
                    preds = classifier_fn(inputs)
                    labels.extend(preds.numpy())
                imgs = []
        if len(imgs) > 0:
            inputs = torch.tensor(np.array(imgs)).permute(0, 3, 1, 2)
            with torch.no_grad():
